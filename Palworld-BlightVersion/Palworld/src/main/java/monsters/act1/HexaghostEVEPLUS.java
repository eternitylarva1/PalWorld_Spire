...
public class HexaghostEVEPLUS extends AbstractMonster {
    ...
    public HexaghostEVEPLUS() {
        super("二火亡魂", "Hexaghost", 250, 20.0F, 0.0F, 450.0F, 450.0F, "images/monsters/theBottom/boss/ghost/core.png");
        ...
        switch (this.nextMove) {
            case 1:
                for(int i = 0; i < 6; ++i) {
                    ...
                    AbstractDungeon.actionManager.addToBottom(new DamageAction(AbstractDungeon.player, (DamageInfo)this.damage.get(2), AttackEffect.BLUNT_HEAVY, true));
                }
...
this.addToBot(new AbstractGameAction() {
    private HexaghostEVEPLUS hexaghost=HexaghostEVEPLUS.this;
    @Override
    public void update() {

        int d = AbstractDungeon.player.currentHealth / 12 + 1;
        if(!MinionHelper.getMinions().monsters.isEmpty()&&config.getBool("Douququ")){
            d=0;
            ArrayList<AbstractMonster> am=MinionHelper.getMinions().monsters.contains(this)?MinionHelper.getMinions().monsters:AbstractDungeon.getCurrRoom().monsters.monsters;

            for (AbstractMonster monster : am) {
                if(monster.currentHealth/12+1>d){
                    d=monster.currentHealth / 12 + 1;
                }
            }
        }

        ((DamageInfo)hexaghost.damage.get(2)).base = d;
        hexaghost.applyPowers();
        hexaghost.setMove((byte)1, Intent.ATTACK, ((DamageInfo)hexaghost.damage.get(2)).base, 6, true);
        isDone=true;
    }
});
    ...
    protected void getMove(int num) {
        if (!this.activated) {
            ...
            int d = AbstractDungeon.player.currentHealth / 12 + 1;
            if(get_pokego.canwork_pokego()>0&&!MinionHelper.getMinions().monsters.contains(this)){
                d=0;
                List<PokeGo> monsters=InfoBlight.getAllRelics(PokeGo.class);
                for (PokeGo monster : monsters) {
                    if(monster.counter/12+1>d){
                        d=monster.counter/12+1;
                    }
                }
            ...
            if(MinionHelper.getMinions().monsters.contains(this)){
                d=0;
                ArrayList<AbstractMonster> am=AbstractDungeon.getCurrRoom().monsters.monsters;
    ...
    public void die() {
        ...
        while(var1.hasNext()) {
            HexaghostOrb orb = (HexaghostOrb)var1.next();
            orb.hide();
        }

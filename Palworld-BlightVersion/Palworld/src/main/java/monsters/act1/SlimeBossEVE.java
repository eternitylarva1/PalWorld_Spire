...
public class SlimeBossEVE extends AbstractMonster {
    private static final Logger logger = LogManager.getLogger(com.megacrit.cardcrawl.monsters.exordium.SlimeBoss.class.getName());
    ...
    private boolean hasdamaged = true;

    public SlimeBossEVE() {
        super("斗蛐蛐专属史莱姆", "SlimeBoss", 140, 0.0F, -30.0F, 400.0F, 350.0F, (String)null, 0.0F, 28.0F);
    ...
    public void takeTurn() {
        switch (this.nextMove) {
            case 1:
                ...
                {
                    this.setMove(SLAM_NAME, (byte)4, Intent.UNKNOWN);
                }else {
            ...
            case 3:
                ...
                if(MinionHelper.hasMinion(this)) {
                    AbstractDungeon.actionManager.addToBottom(new SummonMinionAction(new SpikeSlime_L1(-385.0F, 20.0F, 0, this.currentHealth)));
                    AbstractDungeon.actionManager.addToBottom(new SummonMinionAction(new AcidSlime_L1(120.0F, -8.0F, 0, this.currentHealth)));
                }
                else{
    ...
    public void damage(DamageInfo info) {
        super.damage(info);
        if (!this.isDying && (float)this.currentHealth <= (float)this.maxHealth / 2.0F && this.nextMove != 3&&config.getBool("Douququ")) {
            this.setMove(SPLIT_NAME, (byte)3, Intent.UNKNOWN);
        }

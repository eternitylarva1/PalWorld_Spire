...
public class DamageInfoPatch {
    ...
    public static SpireReturn Prefix(DamageInfo damageInfo, AbstractCreature owner, AbstractCreature target) {
        ...
        if (!owner.isPlayer) {
            ...
            if (damageInfo.owner instanceof AbstractMonster && !getMinionMonsters().contains(damageInfo.owner)) {
                if(AbstractDungeon.player.stance.ID== WrathStance.STANCE_ID&& damageInfo.type== DamageInfo.DamageType.NORMAL){
                    tmp *=2;
                    if (damageInfo.base != (int) tmp) {
                        damageInfo.isModified = true;}}
            }
